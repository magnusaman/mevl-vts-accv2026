...
class SHA_FFN_CRSATTN(torch.nn.Module):
    ...
    def _init_asso_head(self, cfg):
        ...
        self.v7_consensus_matcher = None
        self.v7_content_matcher = None
        dt_cfg = getattr(cfg.MODEL, "DENSETRACK", None)
        if dt_cfg is not None and dt_cfg.ENABLED:
            if dt_cfg.COMP2_ENABLED:
                from ..v7_densetrack import MultiEncoderConsensusMatcher
                enc_dims = dict(zip(dt_cfg.COMP2_ENCODERS, dt_cfg.COMP2_ENCODER_DIMS))
                self.v7_consensus_matcher = MultiEncoderConsensusMatcher(
                    encoder_dims=enc_dims, proj_dim=dt_cfg.COMP2_PROJ_DIM)
                print(f"  [v7] enabled MultiEncoderConsensusMatcher with {len(enc_dims)} encoders")
            if dt_cfg.COMP3_ENABLED:
                from ..v7_densetrack import VLMContentMatcher
                self.v7_content_matcher = VLMContentMatcher(
                    use_mlp_fusion=dt_cfg.COMP3_USE_MLP_FUSION)
                print(f"  [v7] enabled VLMContentMatcher")

        # context container set per-batch by the meta-arch (gom_lstmatcher)
        # so _forward_transformer / _forward_asso can pull encoder features etc.
        self._v7_ctx = None  # dict or None
    ...
    def _forward_transformer(self, proposals, reid_features, query_frame=None, short_term=False):
        ...
        # ---------------- v7 Component 3: VLM content matching ----------------
        # Adds a content-similarity term per layer (cheap, just modifies asso_outputs).
        if (self.v7_content_matcher is not None
                and self._v7_ctx is not None
                and self._v7_ctx.get("frame_texts") is not None):
            try:
                frame_texts = self._v7_ctx["frame_texts"]
                # Build per-proposal text lists: same order as proposals across frames.
                # frame_texts[t] is a list of strings, one per proposal in frame t.
                # Flatten in the order proposals enumerated:
                all_texts = []
                for t, texts_per_frame in enumerate(frame_texts):
                    all_texts.extend(texts_per_frame)
                # query/key text slicing matches box slicing above
                if query_frame is not None:
                    q_texts = frame_texts[query_frame]
                else:
                    q_texts = all_texts
                k_texts = all_texts
                # Wrap each key as (text, conf=1.0) so the content matcher API holds
                k_consensus = [(t, 1.0) for t in k_texts]
                # Apply per-layer
                augmented = []
                for x in asso_outputs:
                    augmented.append(self.v7_content_matcher(x, q_texts, k_consensus))
                asso_outputs = augmented
            except Exception as e:
                print(f"  [v7-WARN] content matcher skipped: {type(e).__name__}: {e}")

        # ---------------- v7 Component 2: multi-encoder consensus ----------------
        # Only fires if comp 2 is enabled AND encoder features were placed on
        # self._v7_ctx by the meta-arch.
        if (self.v7_consensus_matcher is not None
                and self._v7_ctx is not None
                and self._v7_ctx.get("encoder_features") is not None):
            try:
                enc_feats = self._v7_ctx["encoder_features"]  # dict
                image_size = self._v7_ctx.get("image_size", (720, 1280))
                # Per-frame box lists (image coords) from proposals
                boxes_per_frame = []
                for p in proposals:
                    if p.has("proposal_boxes"):
                        boxes_per_frame.append(p.proposal_boxes.tensor)
                    elif p.has("pred_boxes"):
                        boxes_per_frame.append(p.pred_boxes.tensor)
                    else:
                        boxes_per_frame.append(p.gt_boxes.tensor)
                # Build q_boxes that pool to exactly M features, k_boxes to N
                if query_frame is not None:
                    # short-term / single-query-frame: M = n_t[query_frame]
                    q_boxes = [boxes_per_frame[query_frame]]
                else:
                    # long-term / full-clip: M = N = sum(n_t)
                    q_boxes = boxes_per_frame
                k_boxes = boxes_per_frame
                # Augment each layer's asso output
                augmented = []
                for x in asso_outputs:
                    augmented.append(self.v7_consensus_matcher(
                        x, enc_feats, q_boxes, k_boxes, image_size))
                asso_outputs = augmented
            except Exception as e:
                # Defensive: never let v7 break baseline forward
                print(f"  [v7-WARN] consensus matcher skipped: {type(e).__name__}: {e}")
