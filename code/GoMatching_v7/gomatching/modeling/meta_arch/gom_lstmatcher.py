...
class GoMatching(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if dt_cfg is not None and dt_cfg.ENABLED:
            self.v7_cfg = dt_cfg
            if dt_cfg.COMP1_ENABLED:
                from gomatching.modeling.v7_densetrack import SAMProposalAugmenter
                self.v7_proposal_augmenter = SAMProposalAugmenter(
                    clip_dim=dt_cfg.COMP1_CLIP_DIM,
                    hidden_dim=dt_cfg.COMP1_HIDDEN_DIM,
    ...
    def _v7_set_roi_ctx(self, batched_inputs):
        ...
        via roi_heads._v7_ctx. Aggregates per-frame dataloader output into the
        per-clip structures the v7 components expect.

        Per-frame keys the dataloader may attach (via V7Cache.attach):
            v7_encoder_features    : {enc_name: (C, Hf, Wf) tensor}
            v7_sam_polygons        : list[list[float]]
            v7_sam_clip_features   : (M, 1024) tensor
            v7_sam_stability_scores: (M,) tensor
            v7_text                : list[str] per-proposal texts (Comp 3, future)
        """
        if self.v7_cfg is None:
            return
        if not hasattr(self.roi_heads, "_v7_ctx"):
            return
        # Smoke-test bypass: callers can stuff a fully-built ctx here.
        if hasattr(self, "_v7_pending_ctx") and self._v7_pending_ctx is not None:
            self.roi_heads._v7_ctx = self._v7_pending_ctx
            return

        # We need ONE feature map per batched_input per encoder (shape-aligned with
        # boxes_per_frame). Frames whose v7 cache is missing (e.g. dropped by
        # --subsample-stride) get a zero placeholder so torch.stack still gives
        # (T, C, H, W) matching len(batched_inputs). If we used variable-length
        # stacks, roi_align would index past the end and trigger an
        # "illegal memory access" CUDA crash.
        enc_feats = {}              # {enc: [per-frame (C, H, W) tensors or None]}
        sam_polys = []              # list of per-frame polygon lists
        sam_clip_feats = []         # list of per-frame (M, 1024) tensors
        sam_stab = []               # list of per-frame (M,) tensors
        texts = []
        image_size = None

        for x in batched_inputs:
            if image_size is None and "image" in x:
                image_size = tuple(x["image"].shape[-2:])
            if "v7_encoder_features" in x:
                for enc_name, fmap in x["v7_encoder_features"].items():
                    enc_feats.setdefault(enc_name, []).append(fmap)
            sam_polys.append(x.get("v7_sam_polygons", []))
            sam_clip_feats.append(x.get("v7_sam_clip_features",
                                         torch.zeros(0, 1024)))
            sam_stab.append(x.get("v7_sam_stability_scores",
                                   torch.zeros(0)))
            if "v7_text" in x:
                texts.append(x["v7_text"])

        # Per-encoder: pad to exactly len(batched_inputs) frames with zeros.
        # Otherwise we'd silently shape-mismatch boxes_per_frame in the matcher.
        T_expected = len(batched_inputs)
        enc_feats_padded = {}
        for k, v in enc_feats.items():
            if len(v) == 0:
                continue
            template = v[0]
            # Re-walk frames in order, picking real feats or a zero fallback
            walker = iter(v)
            padded = []
            for x in batched_inputs:
                if "v7_encoder_features" in x and k in x["v7_encoder_features"]:
                    padded.append(next(walker))
                else:
                    padded.append(torch.zeros_like(template))
            assert len(padded) == T_expected, (
                f"enc_feats pad mismatch for {k}: "
                f"got {len(padded)} vs expected {T_expected}")
            enc_feats_padded[k] = padded

        ctx = {"image_size": image_size or (720, 1280)}
        if enc_feats_padded:
            ctx["encoder_features"] = {k: torch.stack(v).to(self.device)
                                        for k, v in enc_feats_padded.items()}
        # SAM data only set if at least one frame had non-zero proposals
        if any(len(p) > 0 for p in sam_polys):
            ctx["sam_polygons"] = sam_polys
            ctx["sam_clip_features"] = [t.to(self.device) for t in sam_clip_feats]
            ctx["sam_stability_scores"] = [t.to(self.device) for t in sam_stab]
        if texts:
            ctx["frame_texts"] = texts
        self.roi_heads._v7_ctx = ctx
    ...
    def forward(self, batched_inputs): # forward
        ...
        # ---- v7: stash encoder features / texts / SAM data to roi_heads ----
        self._v7_set_roi_ctx(batched_inputs)
        ...
        if (self.v7_proposal_augmenter is not None
                ...
                and self.roi_heads._v7_ctx.get("sam_polygons") is not None):
            try:
                ctx = self.roi_heads._v7_ctx
                det_proposals = self.v7_proposal_augmenter(
                    det_proposals,
                    ctx["sam_polygons"],          # per-frame list[list[flat poly]]
                    ctx["sam_clip_features"],     # per-frame (M_t, clip_dim)
                    ctx["sam_stability_scores"],  # per-frame (M_t,)
                )
            except Exception as e:
                print(f"  [v7-WARN] proposal augmenter skipped: {type(e).__name__}: {e}")
