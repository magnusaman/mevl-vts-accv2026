...
def _freeze_except_roi_heads(model):
    ...
    try:
        for child in model.module.roi_heads.children():
            ...
            for v in child.parameters():
                v.requires_grad = True
        if hasattr(model.module.roi_heads, 'logit_scale'):
            model.module.roi_heads.logit_scale.requires_grad = True
    except:
        for child in model.roi_heads.children():
            print('unfreezing', child._get_name())
            for v in child.parameters():
                v.requires_grad = True
        if hasattr(model.roi_heads, 'logit_scale'):
            model.roi_heads.logit_scale.requires_grad = True
    # v7 DenseTrack: unfreeze components attached to the meta-arch (not ROI heads)
    m = model.module if hasattr(model, 'module') else model
    for attr in ('v7_proposal_augmenter', 'v7_consensus_matcher', 'v7_content_matcher'):
        if hasattr(m, attr):
            comp = getattr(m, attr)
            if comp is not None:
                print(f'unfreezing v7 component: {attr}')
                for v in comp.parameters():
                    v.requires_grad = True
    return model
